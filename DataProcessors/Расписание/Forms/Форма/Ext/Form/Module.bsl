...
Процедура РасписаниеПередНачаломБыстрогоРедактирования(Элемент, СтандартнаяОбработка)
	...
	ОткрытьФорму("Документ.СоставлениеРасписания.Форма.ФормаДокумента", ПараметрыФормы);
КонецПроцедуры
...
#Область Календарь
	...
	Процедура КалендарьПриВыводеПериода(Элемент, ОформлениеПериода)
		...
		Для Каждого ДатаПериода Из ОформлениеПериода.Даты Цикл
			...
			Если ВыборкаДат.Найти(ДатаПериода.Дата) <> Неопределено Тогда
				ДатаПериода.ЦветФона = WebЦвета.Аквамарин;
			КонецЕсли;
		КонецЦикла;
	КонецПроцедуры
#КонецОбласти
