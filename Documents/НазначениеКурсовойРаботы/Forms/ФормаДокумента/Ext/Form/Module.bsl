	...
	Если Объект.Проведен = Истина Тогда
		...
		Если Ответ = КодВозвратаДиалога.Да Тогда
			...
			Для Каждого Обучающийся Из Объект.Обучающиеся Цикл
				СоздатьПротоколыНаСервере(Обучающийся.Обучающийся, Обучающийся.Тема, Обучающийся.НаПримере);
...
&НаСервере
Процедура СоздатьПротоколыНаСервере(ФИО, Тема, НаПримере)
	НовыйДокумент = Документы.ПротоколКурсовойРаботы.СоздатьДокумент();
	...
	НовыйДокумент.Семестр = Объект.Семестр;
	НовыйДокумент.Дисциплина = Объект.Дисциплина;
	НовыйДокумент.Руководитель = Объект.Руководитель;
